...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-white text-slate-900 relative overflow-hidden border-t border-slate-100">
      <div aria-hidden="true" className="absolute top-0 right-0 w-[600px] h-[600px] bg-primary/5 rounded-full blur-[120px] -z-0 pointer-events-none" />
      <div className="relative max-w-7xl mx-auto px-6 lg:px-12 z-10">
            ...
            <div className="flex justify-center sm:justify-start gap-3">
              {[FaFacebook, FaInstagram, FaLinkedin, FaTwitter].map((Icon, index) => (
                <a key={index} href="#" aria-label={`SERLIMCA ${t('socialMedia')}`} className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center transition-all duration-300 hover:bg-primary hover:border-primary group">
                  <Icon className="w-4 h-4 text-slate-600 group-hover:text-black transition-colors" />
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="text-center sm:text-left">
            ...
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row items-center gap-4 group">
                ...
                <div className="flex flex-col"><span className="text-[10px] uppercase text-slate-600 font-bold tracking-tighter">{t('footer.callUs')}</span><a href="tel:+584265831206" className="text-slate-700 font-bold text-sm hover:text-primary transition-colors">0426-5831206</a></div>
              </div>
              <div className="flex flex-col sm:flex-row items-center gap-4 group">
                ...
                <div className="flex flex-col"><span className="text-[10px] uppercase text-slate-600 font-bold tracking-tighter">{t('footer.writeUs')}</span><a href="mailto:serlimca@gmail.com" className="text-slate-700 font-bold text-sm hover:text-primary transition-colors">serlimca@gmail.com</a></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="py-10">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <p className="text-slate-600 text-[11px] font-medium tracking-wide">© {currentYear} <span className="text-slate-900 font-black">SERVICIOS D&apos;LIMA C.A SERLIMCA</span>. {t('footer.rights')}</p>
            <div className="flex gap-8 text-[10px] uppercase tracking-[0.2em] font-black"><Link to="#" className="text-slate-600 hover:text-black transition-colors">{t('footer.privacy')}</Link><Link to="#" className="text-slate-600 hover:text-black transition-colors">{t('footer.terms')}</Link></div>
          </div>
        </div>
      </div>
    </footer>
